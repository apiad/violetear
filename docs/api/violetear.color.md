...
```python linenums="54"
    def towards(self, other: "Color", percent: float, *, space="hls") -> "Color":
        from_space = dict(rgb=rgb, hls=hls, hsv=hsv)[space]
        to_space = dict(rgb=Color.rgb, hls=Color.hls, hsv=Color.hsv)[space]

        start_values = to_space(self)
        end_values = to_space(other)
